...
module.exports = function(qmat, cvec, amat, bvec, meq = 0, factorized = true) {
    const Dmat = [null].concat(qmat.map(row => [null].concat(row)));
    const dvec = [null].concat(cvec.map(v => -v));
    const Amat = [null].concat(amat.length === 0 ? new Array(qmat.length).fill([]) : amat[0].map((_, i) => [null].concat(amat.map(row => -row[i]))));
