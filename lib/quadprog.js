...
function quadprog(qmat, cvec, amat, bvec, meq = 0, factorized = false) {
  ...
  if (qmat.some(row => n !== row.length)) {
    throw new Error('qmat is not symmetric!');
  } else if (n !== cvec.length) {
    throw new Error('qmat and cvec are incompatible!');
  ...
  if (factorized) {
    // FIXME Check that Q is upper triangular
    for (let j = 0; j < n; j += 1) {
    ...
    if (qmat[0][0] <= 0) {
      throw new Error('qmat is not positive definite!');
    }
    ...
    for (let j = 1; j < n; j += 1) {
      ...
      s = qmat[j][j] - s;
      if (s <= 0) {
        throw new Error('qmat in quadratic function is not positive definite!');
      }
